...
import {
  ...
  TrendingUp, TrendingDown, Facebook, Instagram, Globe, Lightbulb,
...
const PLATFORM_META = {
  ...
  google_business: { name: 'Google Business', color: '#4285F4', icon: Globe },
};

const INSIGHT_ICON = { good: CheckCircle, short: AlertCircle, few: AlertCircle, long: AlertCircle, many: AlertCircle, consider_better_time: Info };
...
export default function PostPerformance() {
  ...
  if (!data) {
    return (
      <Layout title="Post Performance">
        <Card><EmptyState icon={TrendingUp} title="Post not found" subtitle="This post may have been deleted" /></Card>
      </Layout>
    );
  }
  ...
  return (
    <Layout
      ...
      action={<Button variant="ghost" onClick={() => router.push('/analytics')}><ArrowLeft size={14} /> Analytics</Button>}
    >
      {/* PREVIEW + COMPARISON */}
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {[
              { label: 'Likes', key: 'likes', icon: Heart, color: '#F43F5E' },
              { label: 'Comments', key: 'comments', icon: MessageCircle, color: t.primary },
              { label: 'Shares', key: 'shares', icon: Share2, color: t.success },
              ...
              return (
                <div key={key} style={{ background: t.input, borderRadius: 12, padding: 16, border: `1px solid ${t.border}` }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    {isAbove ? <TrendingUp size={12} style={{ color: t.success }} /> : <TrendingDown size={12} style={{ color: t.error }} />}
      ...
      <Card style={{ marginBottom: 20 }}>
        <SectionHeader icon={TrendingUp} title="Performance by platform" />
        {platformMetrics.length === 0 ? (
          <EmptyState icon={Globe} title="No platform data yet" subtitle="Connect your social accounts to see per-platform engagement" />
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(3, platformMetrics.length)}, 1fr)`, gap: 16 }}>
            {platformMetrics.map((m) => {
              const meta = PLATFORM_META[m.platform] || { name: m.platform, icon: Globe, color: t.primary };
              const Icon = meta.icon;
                ...
                <div key={m.platform} style={{ background: t.input, borderRadius: 12, padding: 18, border: `1px solid ${t.border}` }}>
                  ...
                  {m.note ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {[
                        { label: 'Likes', value: m.likes, icon: Heart, color: '#F43F5E' },
                        { label: 'Comments', value: m.comments, icon: MessageCircle, color: t.primary },
                        { label: 'Shares', value: m.shares, icon: Share2, color: t.success },
                        { label: 'Reach', value: m.reach, icon: Eye, color: t.info },
                        { label: 'Impressions', value: m.impressions, icon: Eye, color: t.textMuted },
      ...
      {insights.length > 0 && (
        <Card style={{ marginBottom: 20 }}>
          <SectionHeader icon={Lightbulb} title="Post insights" />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
            {insights.map((insight) => {
              const Icon = INSIGHT_ICON[insight.assessment] || Info;
              return (
      ...
      {data.timeline && data.timeline.length > 0 && (
        <Card>
          <SectionHeader icon={TrendingUp} title="Engagement timeline" />
