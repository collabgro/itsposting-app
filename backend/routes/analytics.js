...
module.exports = (pool) => {
  ...
  router.get('/overview', async (req, res) => {
    try {
      ...
      const overviewParams = [req.customerId, String(days)];
      const topParams     = [req.customerId];
      let overviewPF = '';
      let topPF      = '';

      if (platform && platform !== 'all') {
        overviewParams.push(platform);
        topParams.push(platform);
        const pfClause = (n) => ` AND (platform = $${n} OR (platforms IS NOT NULL AND (platforms::jsonb) ? $${n}))`;
        overviewPF = pfClause(overviewParams.length);
        topPF      = pfClause(topParams.length);
      }
  ...
  router.get('/posts', async (req, res) => {
    try {
      const { sort = 'recent', platform, contentType, status, period, limit = 100 } = req.query;
      const safeLimit = Math.min(Math.max(parseInt(limit) || 100, 1), 200);

      const SAFE_SORTS = {
        recent: 'p.created_at DESC',
        best:   'p.performance_score DESC NULLS LAST, p.created_at DESC',
        oldest: 'p.created_at ASC',
      };
      const orderBy = SAFE_SORTS[sort] || SAFE_SORTS.recent;

      let query = `
        SELECT p.id, p.content_type, p.caption, p.media_url, p.status,
               p.scheduled_date, p.posted_at, p.engagement,
               p.engagement_by_platform, p.performance_score, p.platforms, p.platform,
               p.source, p.created_at, p.credits_used, p.last_metrics_sync
        FROM posts p
      ...
      if (contentType && contentType !== 'all') {
        pIdx++; query += ` AND p.content_type = $${pIdx}`; params.push(contentType);
      }
      if (platform && platform !== 'all') {
        pIdx++;
        query += ` AND (p.platform = $${pIdx} OR (p.platforms IS NOT NULL AND (p.platforms::jsonb) ? $${pIdx}))`;
        params.push(platform);
      }
      if (status && status !== 'all') {
        pIdx++; query += ` AND p.status = $${pIdx}`; params.push(status);
      }
      if (period && period !== 'all') {
        const days = Math.min(Math.max(parseInt(period) || 30, 1), 365);
        pIdx++; query += ` AND p.created_at > NOW() - ($${pIdx} || ' days')::INTERVAL`; params.push(String(days));
      }
  ...
  router.get('/content-performance', async (req, res) => {
    try {
      // Only count known content types — avoids wizard theme values (e.g. 'just_finished_job')
      // leaking into this chart as phantom rows.
      const KNOWN_TYPES = ['static', 'photo', 'carousel', 'video'];
      ...
      const result = await pool.query(`
        SELECT
          ...
          COUNT(*)                                                          AS total,
          COUNT(*) FILTER (WHERE status = 'posted')                        AS posted,
          COALESCE(AVG((engagement->>'likes')::numeric)    FILTER (WHERE status = 'posted' AND last_metrics_sync IS NOT NULL), 0) AS avg_likes,
          COALESCE(AVG((engagement->>'comments')::numeric) FILTER (WHERE status = 'posted' AND last_metrics_sync IS NOT NULL), 0) AS avg_comments,
          COALESCE(AVG((engagement->>'shares')::numeric)   FILTER (WHERE status = 'posted' AND last_metrics_sync IS NOT NULL), 0) AS avg_shares,
        ...
        ORDER BY avg_score DESC
      `, [req.customerId, KNOWN_TYPES]);

      // Day-of-week posting distribution — only count days you actually published
  ...
  router.get('/streak', async (req, res) => {
    try {
      ...
      const lastPostedAt = row?.last_posted_at || null;
      let streak         = row?.posting_streak  || 0;

      // Lazy-reset: if the customer hasn't posted in 48+ hours, the streak is broken
      if (streak > 0 && lastPostedAt) {
        const hoursSince = (Date.now() - new Date(lastPostedAt).getTime()) / 3600000;
        if (hoursSince > 48) {
          streak = 0;
          pool.query(
            `UPDATE customers SET posting_streak = 0 WHERE id = $1`,
            [req.customerId]
          ).catch(e => console.warn('[streak] lazy reset failed:', e.message));
        }
      }
