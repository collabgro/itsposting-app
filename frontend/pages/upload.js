...
import {
  Upload as UploadIcon, Image as ImageIcon, Layers, Video,
  Calendar as CalendarIcon, Save, Facebook, Instagram, Globe,
  Zap, FolderOpen, X, Loader2, Sparkles,
} from 'lucide-react';
...
const CONTENT_TYPES = [
  ...
  { id: 'carousel', label: 'Carousel', icon: Layers, desc: '2-10 images' },
  { id: 'video', label: 'Video', icon: Video, desc: '1 video' },
];

const PLATFORMS = [
  { id: 'facebook', name: 'Facebook', icon: Facebook },
  { id: 'instagram', name: 'Instagram', icon: Instagram },
  { id: 'google_business', name: 'Business Profile', icon: Globe },
];
...
export default function Upload() {
  ...
  return (
    <Layout title="Create Post" subtitle="Upload your own content" action={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Button variant="secondary" onClick={() => setShowAIModal(true)}><Sparkles size={14} style={{ color: t.primary }} /> Create</Button><Button variant="secondary" onClick={() => router.push('/calendar')}>View Calendar</Button></div>}>
      <div style={{ maxWidth: 720, margin: '0 auto', width: '100%' }}>
        <div style={{ padding: '14px 18px', background: t.primaryBg, border: `1px solid ${t.primaryBorder}`, borderRadius: 10, marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          ...
          <Button variant="primary" size="sm" onClick={() => setShowAIModal(true)}><Sparkles size={14} /> Open Creator</Button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Card style={{ marginBottom: 16 }}><SectionHeader icon={ImageIcon} title={`Upload ${contentType === 'carousel' ? 'Files (2-10)' : 'File'}`} /><div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}><button type="button" onClick={() => fileInputRef.current?.click()} style={{ flex: '1 1 220px', padding: 14, background: t.input, border: `2px dashed ${t.borderStrong}`, borderRadius: 8, color: t.textSecondary, fontSize: 13, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><UploadIcon size={16} strokeWidth={2} /> Upload from device</button><button type="button" onClick={openLibrary} style={{ flex: '1 1 220px', padding: 14, background: t.primaryBg, border: `1px solid ${t.primaryBorder}`, borderRadius: 8, color: t.primary, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><FolderOpen size={16} strokeWidth={2} /> Choose from library</button></div><input ref={fileInputRef} type="file" accept={contentType === 'video' ? 'video/*' : 'image/*'} multiple={contentType === 'carousel'} onChange={handleFileSelect} style={{ display: 'none' }} />{previews.length > 0 && <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 8 }}>{previews.map((url, idx) => <div key={idx} style={{ aspectRatio: '1/1', borderRadius: 8, overflow: 'hidden', background: t.input, border: `1px solid ${t.border}`, position: 'relative' }}>{contentType === 'video' ? <video src={url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} controls /> : <img src={url} alt={`preview ${idx + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}{files[idx]?.libraryFileId && <div style={{ position: 'absolute', bottom: 4, right: 4, padding: '2px 6px', background: 'rgba(124,92,252,0.9)', borderRadius: 4, fontSize: 9, color: '#fff', fontWeight: 600 }}>LIB</div>}</div>)}</div>}</Card>
          <Card style={{ marginBottom: 16 }}><SectionHeader icon={Save} title="Caption & Hashtags" /><div style={{ marginBottom: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6, gap: 8, flexWrap: 'wrap' }}><label style={{ fontSize: 12, fontWeight: 600, color: t.textSecondary, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Caption</label></div><Textarea value={caption} onChange={(e) => setCaption(e.target.value)} placeholder="Write a caption..." rows={5} /></div><div style={{ marginBottom: 12 }}><label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: t.textSecondary, marginBottom: 6 }}>Hashtags</label><Input value={hashtags} onChange={(e) => setHashtags(e.target.value)} placeholder="#social #marketing" /></div></Card>
