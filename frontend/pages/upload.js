...
export default function Upload() {
  ...
  return (
    <Layout title="Create Post" subtitle="Upload your own content" action={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Button variant="secondary" onClick={() => router.push('/wizard')}><IpSparkle size={14} color="url(#brand-gradient)" /> Create</Button><Button variant="secondary" onClick={() => router.push('/calendar')}>View Calendar</Button></div>}>
      <div style={{ maxWidth: 720, margin: '0 auto', width: '100%' }}>
        ...
        <form onSubmit={handleSubmit}>
              ...
              {PLATFORMS.map(({ id, name, icon: Icon }) => {
                ...
                return (
                  <button
                    ...
                    style={{ padding: '12px 10px', border: `2px solid ${sel ? t.primary : t.border}`, background: sel ? t.primaryBg : t.input, borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, transition: 'all 150ms' }}
                  >
                    <Icon size={16} style={{ color: sel ? t.primary : t.textMuted, flexShrink: 0 }} />
                    <span style={{ fontSize: 13, fontWeight: 600, color: sel ? t.text : t.textMuted }}>{name}</span>
                    {sel && <div style={{ marginLeft: 'auto', width: 16, height: 16, borderRadius: '50%', background: t.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          <Card style={{ marginBottom: 16 }}><SectionHeader icon={CalendarIcon} title="Schedule" /><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><Button type="button" variant={scheduleMode === 'now' ? 'primary' : 'secondary'} onClick={() => setScheduleMode('now')}>Save as draft</Button><Button type="button" variant={scheduleMode === 'later' ? 'primary' : 'secondary'} onClick={() => setScheduleMode('later')}>Schedule for later</Button></div>{scheduleMode === 'later' && <div style={{ marginTop: 12 }}><Input type="datetime-local" value={scheduledDate} onChange={(e) => setScheduledDate(e.target.value)} /></div>}</Card>
